...
def test_salient_excerpt(loudness_cutoff):
    ...
    with tempfile.NamedTemporaryFile(suffix=".wav") as f:
        ...
        signal[..., sr * 20 : sr * 21] = MAP[loudness_cutoff] * paddle.randn([44100])
...
def test_stft(window_length, hop_length, window_type):
    ...
    for _stft_params in [None, stft_params]:
        ...
        assert np.allclose(signal.stft_data.cpu().numpy(), stft_data.cpu().numpy())
        ...
        assert np.allclose(recon_stft.cpu().numpy(), signal.stft_data.cpu().numpy())
