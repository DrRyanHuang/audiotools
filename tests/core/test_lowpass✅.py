...
from audiotools.core import lowpass_filter
from audiotools.core import LowPassFilter
from audiotools.core import LowPassFilters
from audiotools.core import resample_frac
...
class TestLowPassFilters(_BaseTest):
    ...
    def test_same_as_downsample(self):
        for _ in range(10):
            ...
            np.random.seed(1234)
            x = paddle.to_tensor(np.random.randn(2 * 3 * 4 * 100), dtype="float32")
            rolloff = 0.945
            for old_sr in [2, 3, 4]:
